...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw java.util.NoSuchElementException("can't calculate max")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
    val b = left.filterAcc(p, acc)
    val c = right.filterAcc(p, b)
    if p(elem) then c.incl(elem) else c
  ...
  def union(that: TweetSet): TweetSet =
    that.filterAcc(x => !this.contains(x), this)

  def mostRetweeted: Tweet = 
    filter(p => p.retweets > elem.retweets) match 
      case _: Empty => elem
      case candidate: NonEmpty => candidate.mostRetweeted
...
object GoogleVsApple:
  ...
  def filterTweets(tags: List[String]): TweetSet = 
    TweetReader.allTweets.filter(x => tags.exists(y => x.text.contains(y)))
