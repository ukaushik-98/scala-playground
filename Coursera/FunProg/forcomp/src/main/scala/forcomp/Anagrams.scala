...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.groupBy((c) => c.toLower).toList.map((c, s) => (c, s.length)).sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences match
      case Nil => List(Nil)
      case (char, count) :: next =>
        combinations(next).flatMap(x => (0 to count).map(y =>
          if y == 0 then x else  (char, y) :: x
        ))
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    ...
    x.foldLeft(List.empty[(Char, Int)]){
      (accum, z) => accum.appended((z._1, z._2 - my.getOrElse(z._1, 0)))
    }.filter(_._2 > 0)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] =
    def iter(occ: Occurrences): List[Sentence] =
      if occ.isEmpty then List(Nil)
        ...
        combinations(occ).flatMap { combo =>
          dictionaryByOccurrences.getOrElse(combo, Nil).flatMap { word =>
            iter(subtract(occ, combo)).map { rest =>
              word :: rest
            }
          }
        }
